...
BallerEngine::Camera::Camera(Vector3 position, Vector3 target, float fov) {
...
void BallerEngine::Camera::_moveUpward() {
  ...
  if (IsKeyDown(KEY_E)) {
    camera.target =
        Vector3Add(camera.target, Vector3Scale(camera.up, sensitivity));
    camera.position =
        Vector3Add(camera.position, Vector3Scale(camera.up, sensitivity));
  }
  if (IsKeyDown(KEY_Q)) {
    camera.target =
        Vector3Subtract(camera.target, Vector3Scale(camera.up, sensitivity));
    camera.position =
        Vector3Subtract(camera.position, Vector3Scale(camera.up, sensitivity));
  }
}
...
void BallerEngine::Camera::_panVertical() {
  ...
  if (deltaY != 0.0f) {
    camera.target = Vector3Add(camera.target,
                               Vector3Scale(camera.up, -deltaY * sensitivity));
    // camera.up = Vector3Normalize(Vector3CrossProduct(_getCameraRight(),
    // _getCameraForward()));
  }
}
...
void BallerEngine::Camera::_panHorizontal() {
  ...
  if (deltaX != 0.0f) {
    ...
    camera.target =
        Vector3Add(camera.target, Vector3Scale(right, deltaX * sensitivity));
  }
}
...
GameCamera::GameCamera(Vector3 position, Vector3 target, Car *carObject) {
  camera.position = position;
  camera.target = target;
  camera.projection = CAMERA_PERSPECTIVE;
  camera.fovy = 45.0f;
...
void GameCamera::updateCamera() {
  camera.position =
      Vector3{car->getOrigin().x() - car->getForwardVector().x() * 5,
              2 + car->getOrigin().y(),
              car->getOrigin().z() - car->getForwardVector().z() * 5};
  btVector3 forward = car->getForwardVector();
  camera.target = Vector3{camera.position.x + forward.x(), camera.position.y,
